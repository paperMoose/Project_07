	...
	if(solid == 0){//framewire
		if(divisions == 0){//addlines to module
			...
			for(i=0; i<4; i++){//top left surface[0]
				for(j=0; j<4; j++){
					point_copy(&surface[0].ctrl[i][j], &final[j][i]);
				}
			}

			for(i=3; i<7; i++){//top right surface[1]
				for(j=0; j<4; j++){
					point_copy(&surface[1].ctrl[i-3][j], &final[j][i]);
				}
			}

			for(i=0; i<4; i++){//bottom left surface[2]
				for(j=3; j<7; j++){
					point_copy(&surface[2].ctrl[i][j-3], &final[j][i]);
				}
			}

			for(i=3; i<7; i++){//lower right surface[0]
				for(j=3; j<7; j++){
					point_copy(&surface[3].ctrl[i-3][j-3], &final[j][i]);
				}
			}
...
void module_sphere(Module *md){
	if(NULL != md){
		...
		int i,j;
		Point cntrl[4];
		point_set3D(&cntrl[0],0,0,0);
		point_set3D(&cntrl[1],3,1,0);
		point_set3D(&cntrl[2],3,3,0);
		point_set3D(&cntrl[3],3,3,0);

		BezierCurve curve;
		bezierCurve_init(&curve);
		bezierCurve_set(&curve,cntrl);
		module_bezierCurve(md,&curve,3);
		matrix_identity(&mat);

		for ( j = 0; j < 8; j++)
		{
			matrix_rotateY(&mat, cos(45*M_PI/180), sin(45*M_PI/180));
			for ( i = 0; i < 4; i++)
			{
				matrix_xformPoint(&mat,&cntrl[i],&cntrl[i]);
			}
			bezierCurve_set(&curve,cntrl);
			module_bezierCurve(md,&curve,3);
		}
		matrix_identity(&mat);
		matrix_scale(&mat,1,-1,1);
		matrix_translate(&mat,0,6,0);
